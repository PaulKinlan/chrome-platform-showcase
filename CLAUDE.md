...
A premium hand-crafted demo for every web platform feature shipping in Chrome, one per API, each
with its own portfolio of interactive use cases. (Per-release "uber" demos were removed on
2026-08-27 — see migrations.json — so effort goes to per-feature coverage.) Generated by a remote
Claude Code routine on a daily cron (plus a weekly hardening routine), reviewed and merged by
humans. Hosted on Deno Deploy via Deno's native GitHub integration.
